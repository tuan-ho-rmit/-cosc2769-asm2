...
const GroupMain = () => {
    ...
    const [groupDescription, setGroupDescription] = useState('');  // group discription
    const [groupAvatar, setGroupAvatar] = useState('');  // group img
    const navigate = useNavigate();  // Navigate when clicking on View Members
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            reader.onload = (e) => {
                ...
                if (images.length === files.length) {
                    // update when file is finished loaging
                    setImages(images);
                }
            };
    ...
    const handleViewMembers = () => {
        navigate(`/viewmembers/${groupId}`);  // move to viewmembers path
    };
    ...
    function handleAddPost() {
        ...
        fetch(`http://localhost:3000/api/groups/${groupId}/posts`, {
            ...
            .then(newPost => {
                ...
                setPostList([newPost, ...posts]); // update
                setContent("");
                setImages([]); // reset image sets
            })
    ...
    function handleEditPost(id) {
        ...
        if (updatedContent !== null) {
            ...
            .then(async updatedPost => {
                // update current status without calling from database
